...
def chart_node(state: MaingraphState):
    """图表排版Agent - 负责图片、表格、图表标题和编号格式"""
    system_messages = [
        SystemMessage(content="<-- 进入 07 图表 Agent 排版 Node -->")
    ]
    ...
    system_prompt = """
    你是一个专业的图表排版大师，负责将排版任务转换为代码参数，以支持后续的工具调用。

    ## 你的职责范围
    - 图表标题格式（如"图1 xxx"、"表1 xxx"）
    - 图片编号和对齐
    - 表格标题和内容格式
    - 图注、表注的字体和字号
    ...
    return {
        ...
        "messages": system_messages + [
            AIMessage(
                content=f"【Chart Agent】执行完成：{overall_status}，成功 {success_count}/{total_tasks} 项"
            )
        ],
    }
