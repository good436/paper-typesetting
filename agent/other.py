...
def other_node(state: MaingraphState):
    """兜底排版Agent - 负责所有未被其他Agent分类的排版任务"""
    system_messages = [
        SystemMessage(content="<-- 进入 07 兜底 Agent 排版 Node -->")
    ]
    ...
    system_prompt = """
    你是一个通用的论文排版大师，负责处理各种未被特定Agent处理的排版任务。

    ## 你的职责范围
    - 处理所有未被 cover/text/author/chart/fund/reference 覆盖的排版任务
    - 包括但不限于：中图分类号、页眉页脚、页码格式、附录等
    ...
    return {
        ...
        "messages": system_messages + [
            AIMessage(
                content=f"【Other Agent】执行完成：{overall_status}，成功 {success_count}/{total_tasks} 项"
            )
        ],
    }
