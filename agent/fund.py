...
def fund_node(state: MaingraphState):
    """基金项目排版Agent - 负责课题或基金项目信息的格式化"""
    system_messages = [
        SystemMessage(content="<-- 进入 07 基金项目 Agent 排版 Node -->")
    ]
    ...
    system_prompt = """
    你是一个专业的基金项目排版大师，负责将排版任务转换为代码参数，以支持后续的工具调用。
    ...
    return {
        ...
        "messages": system_messages + [
            AIMessage(
                content=f"【Fund Agent】执行完成：{overall_status}，成功 {success_count}/{total_tasks} 项"
            )
        ],
    }
